...
""")
    ...
    SELECT new com.ddo.torneios.dto.AgregadoCasaForaDTO(
        SUM(CASE WHEN m.jogador.id = :jogadorId AND cm.ligaClube <> 'SELECAO' AND p.golsMandante > p.golsVisitante THEN 1L ELSE 0L END),
        SUM(CASE WHEN m.jogador.id = :jogadorId AND cm.ligaClube <> 'SELECAO' AND p.golsMandante = p.golsVisitante THEN 1L ELSE 0L END),
        SUM(CASE WHEN m.jogador.id = :jogadorId AND cm.ligaClube <> 'SELECAO' AND p.golsMandante < p.golsVisitante THEN 1L ELSE 0L END),
        SUM(CASE WHEN m.jogador.id = :jogadorId AND cm.ligaClube = 'SELECAO' AND p.golsMandante > p.golsVisitante THEN 1L ELSE 0L END),
        SUM(CASE WHEN m.jogador.id = :jogadorId AND cm.ligaClube = 'SELECAO' AND p.golsMandante = p.golsVisitante THEN 1L ELSE 0L END),
        SUM(CASE WHEN m.jogador.id = :jogadorId AND cm.ligaClube = 'SELECAO' AND p.golsMandante < p.golsVisitante THEN 1L ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :jogadorId AND cv.ligaClube <> 'SELECAO' AND p.golsVisitante > p.golsMandante THEN 1L ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :jogadorId AND cv.ligaClube <> 'SELECAO' AND p.golsVisitante = p.golsMandante THEN 1L ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :jogadorId AND cv.ligaClube <> 'SELECAO' AND p.golsVisitante < p.golsMandante THEN 1L ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :jogadorId AND cv.ligaClube = 'SELECAO' AND p.golsVisitante > p.golsMandante THEN 1L ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :jogadorId AND cv.ligaClube = 'SELECAO' AND p.golsVisitante = p.golsMandante THEN 1L ELSE 0L END),
        ...
        SUM(CASE WHEN m.jogador.id = :jogadorId THEN p.golsMandante ELSE 0L END),
        SUM(CASE WHEN m.jogador.id = :jogadorId THEN p.golsVisitante ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :jogadorId THEN p.golsVisitante ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :jogadorId THEN p.golsMandante ELSE 0L END)
    )
    FROM Partida p
    JOIN p.mandante m
    JOIN m.clube cm
    ...
    SELECT new com.ddo.torneios.dto.AgregadoCasaForaParDTO(
        ...
        SUM(CASE WHEN m.jogador.id = :id1 THEN p.golsMandante ELSE 0L END),
        SUM(CASE WHEN m.jogador.id = :id1 THEN p.golsVisitante ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :id1 THEN p.golsVisitante ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :id1 THEN p.golsMandante ELSE 0L END),

        SUM(CASE WHEN m.jogador.id = :id2 THEN p.golsMandante ELSE 0L END),
        SUM(CASE WHEN m.jogador.id = :id2 THEN p.golsVisitante ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :id2 THEN p.golsVisitante ELSE 0L END),
        SUM(CASE WHEN v.jogador.id = :id2 THEN p.golsMandante ELSE 0L END)
    )
    FROM Partida p
    JOIN p.mandante m
    JOIN m.clube cm
